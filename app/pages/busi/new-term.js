...
function serializeDetailForm() {
	var inserted = [];
	var data = {};
    $('#detail-fm').find('div > input').each(function(index,element){
		var field_attr = getObjIndex("field_attr",$(element).attr('name'));
		inserted.push($.extend({},{
							'field_id': field_attr?field_attr.field_id:'',
							'src'	: "",
							'update': $(element).val()}))
	});
	data.insert = inserted;
    return data;
}
...
function submitDetail() {
    ...
    var url = '/action/ms/changelog/create-new-term';
	var data = serializeDetailForm();
	data.apppay_id = query.apppay_id;
...
$(function() {
    ...
    if (query.operateType == 'create') {
        ...
        $$.request('/action/ms/changelog/view', query, function(data) {
            if (data.errcode == 0) {
                loadDetailForm(data);
            }
        });
